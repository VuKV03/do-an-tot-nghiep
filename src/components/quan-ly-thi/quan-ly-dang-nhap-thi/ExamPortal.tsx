...
export default function ExamPortal({ currentUser, subject, onLogout, onExamStart, mode = 'taking', previewExamData }: ExamPortalProps) {
  ...
  const fetchSessionInfo = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/exam/portal/me/exam-info?candidate_id=${currentUser.id}&subject=${encodeURIComponent(subject)}`, {
        headers: {
  ...
  const handleConfirmStart = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${API_BASE_URL}/exam/portal/me/confirm-start?result_id=${sessionInfo?.result_info?.id}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleSubmitDraft = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${API_BASE_URL}/exam/portal/submit-draft?result_id=${sessionInfo?.result_info?.id}`, {
        method: 'POST',
        headers: {
  ...
  const doSubmit = async (isAuto: boolean) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${API_BASE_URL}/exam/portal/submit-final?result_id=${sessionInfo?.result_info?.id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
