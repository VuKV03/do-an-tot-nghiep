...
import { dmLoaiHinhCauHoiApi, type DmLoaiHinhCauHoiAPI } from '../../../services/danhMucApi.ts';

const { RangePicker } = DatePicker;

/** Type khớp hoàn toàn với DB / API fields */
export interface DmLoaiHinhCauHoiType {
  id: string;
  code: string;
  name: string;
  note?: string;
...
// ─── Inline modals ──────────────────────────────────────────────────
function CreateModal({ open, onClose, onSave }: { open: boolean; onClose: () => void; onSave: (v: Partial<DmLoaiHinhCauHoiType>) => Promise<boolean> }) {
  const [form, setForm] = React.useState<Partial<DmLoaiHinhCauHoiType>>({});
...
function UpdateModal({ open, onClose, record, onSave }: { open: boolean; onClose: () => void; record: DmLoaiHinhCauHoiType | null; onSave: (v: Partial<DmLoaiHinhCauHoiType>) => Promise<boolean> }) {
  const [form, setForm] = React.useState<Partial<DmLoaiHinhCauHoiType>>({});
...
function DetailModal({ open, onClose, record }: { open: boolean; onClose: () => void; record: DmLoaiHinhCauHoiType | null }) {
  return open && record ? (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-lg shadow-xl w-[560px] p-6">
        <div className="text-xl font-semibold text-slate-800 pb-3 border-b border-gray-200 mb-4">Chi tiết loại hình câu hỏi</div>
        <div className="flex flex-col gap-4">
          {([['id','id'],['code','Mã loại hình'],['name','Tên loại hình'],['note','Ghi chú'],['created_at','Ngày tạo'],['updated_at','Ngày cập nhật']] as [keyof DmLoaiHinhCauHoiType, string][]).map(([key, label]) => (
...
export default function DanhMucLoaiHinhCauHoi() {
  const [data, setData] = useState<DmLoaiHinhCauHoiType[]>([]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<DmLoaiHinhCauHoiType | null>(null);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try { const res = await dmLoaiHinhCauHoiApi.list(); setData(res.data as DmLoaiHinhCauHoiType[]); }
  ...
  const columns: ColumnsType<DmLoaiHinhCauHoiType> = [
