...
import { dmCapDoTuDuyApi, type DmCapDoTuDuyAPI } from '../../../services/danhMucApi.ts';

const { RangePicker } = DatePicker;

/** Type khớp hoàn toàn với DB / API fields */
export interface DmCapDoTuDuyType {
  id: string;
  code: string;
  name: string;
  note?: string;
...
function CreateModal({ open, onClose, onSave }: { open: boolean; onClose: () => void; onSave: (v: Partial<DmCapDoTuDuyType>) => Promise<boolean> }) {
  const [form, setForm] = React.useState<Partial<DmCapDoTuDuyType>>({});
...
function UpdateModal({ open, onClose, record, onSave }: { open: boolean; onClose: () => void; record: DmCapDoTuDuyType | null; onSave: (v: Partial<DmCapDoTuDuyType>) => Promise<boolean> }) {
  const [form, setForm] = React.useState<Partial<DmCapDoTuDuyType>>({});
...
function DetailModal({ open, onClose, record }: { open: boolean; onClose: () => void; record: DmCapDoTuDuyType | null }) {
  return open && record ? (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-lg shadow-xl w-[560px] p-6">
        <div className="text-xl font-semibold text-slate-800 pb-3 border-b border-gray-200 mb-4">Chi tiết cấp độ tư duy</div>
        <div className="flex flex-col gap-4">
          {([['id','id'],['code','Mã cấp độ'],['name','Tên cấp độ'],['note','Ghi chú'],['created_at','Ngày tạo'],['updated_at','Ngày cập nhật']] as [keyof DmCapDoTuDuyType, string][]).map(([key, label]) => (
...
export default function DanhMucCapDoTuDuy() {
  const [data, setData] = useState<DmCapDoTuDuyType[]>([]);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await dmCapDoTuDuyApi.list();
      setData(res.data as DmCapDoTuDuyType[]);
  ...
  return (
    <ConfigProvider theme={{ token: { colorPrimary: '#1d4ed8', borderRadius: 6 }, components: { Table: { headerBg: '#f8fafc', headerColor: '#334155', rowHoverBg: '#f1f5f9' } } }}>
      ...
      <CreateModal open={isCreateOpen} onClose={() => setIsCreateOpen(false)}
        onSave={async (v) => {
          try { await dmCapDoTuDuyApi.create({ code: v.code!, name: v.name!, note: v.note ?? '' }); messageApi.success('Thêm thành công!'); fetchData(); return true; }
      ...
      <UpdateModal open={isUpdateOpen} onClose={() => setIsUpdateOpen(false)} record={selectedRecord}
        onSave={async (v) => {
          ...
          try { await dmCapDoTuDuyApi.update(selectedRecord.id, { code: v.code, name: v.name, note: v.note }); messageApi.success('Cập nhật thành công!'); fetchData(); return true; }
      ...
      <DeleteModal open={isDeleteOpen} onClose={() => setIsDeleteOpen(false)} itemName={selectedRecord?.name} isMultiple={isDeleteMultiple} multipleCount={selectedRowKeys.length}
        onConfirm={async () => {
          try {
            if (isDeleteMultiple) { await Promise.all(selectedRowKeys.map((k) => dmCapDoTuDuyApi.delete(String(k)))); messageApi.success(`Đã xóa ${selectedRowKeys.length} cấp độ!`); setSelectedRowKeys([]); }
            else if (selectedRecord) { await dmCapDoTuDuyApi.delete(selectedRecord.id); messageApi.success('Đã xóa!'); }
            fetchData();
