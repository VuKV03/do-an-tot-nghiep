...
export interface DeleteCapDoTuDuyModalProps {
  open: boolean;
  onClose: () => void;
  onConfirm: () => void;
  itemName?: string;
...
export default function DeleteCapDoTuDuyModal({ open, onClose, onConfirm, itemName, isMultiple, multipleCount }: DeleteCapDoTuDuyModalProps) {
  return (
    <Modal
      title={
      ...
      <div className="flex items-start gap-4 mb-8 px-2">
        ...
        <div className="text-[17px] text-slate-800 pt-1">
          {isMultiple
            ...
            : `Bạn có chắc chắn xóa bản ghi có tên cấp độ tư duy "${itemName || ''}"?`
          }
        </div>
      </div>
