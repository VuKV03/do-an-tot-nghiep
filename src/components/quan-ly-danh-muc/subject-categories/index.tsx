...
import { Button, ConfigProvider, DatePicker, Empty, Input, Select, Space, Table, message, Spin } from 'antd';
import type { ColumnsType } from 'antd/es/table';
import { ChevronDown, ChevronUp, Edit, Eye, Settings, Trash2 } from 'lucide-react';
...
import { dmMonHocApi, type DmMonHocAPI } from '../../../services/danhMucApi.ts';
...
export default function DanhMucMonHoc() {
  const [data, setData] = useState<DmMonHocType[]>([]);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await dmMonHocApi.list();
      // API trả về đúng tên field → dùng thẳng, không cần convert
      setData(res.data as DmMonHocType[]);
    } catch {
  ...
  const handleOpenDetail = (record: DmMonHocType) => { setSelectedRecord(record); setIsDetailModalOpen(true); };
  const handleOpenUpdate = (record: DmMonHocType) => { setSelectedRecord(record); setIsUpdateModalOpen(true); };
  const handleOpenDelete = (record: DmMonHocType) => { setSelectedRecord(record); setIsDeleteMultiple(false); setIsDeleteModalOpen(true); };
  const handleOpenDeleteMultiple = () => { setIsDeleteMultiple(true); setIsDeleteModalOpen(true); };
  const handleOpenCauHinh = (record: DmMonHocType) => { setSelectedRecord(record); setIsCauHinhModalOpen(true); };
  ...
  const columns: ColumnsType<DmMonHocType> = [
    ...
    { title: 'Ngày tạo', dataIndex: 'created_at', key: 'created_at', render: (v) => v ? new Date(v).toLocaleString('vi-VN') : '' },
    {
      ...
      render: (v: boolean) => (
        <span className={`px-3 py-1 rounded border text-sm font-medium ${v ? 'border-emerald-400 text-emerald-600 bg-emerald-50' : 'border-rose-400 text-rose-500 bg-rose-50'}`}>
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small">
          <Button type="text" onClick={() => handleOpenDetail(record)} icon={<Eye size={16} className="text-blue-600" />} className="bg-blue-50 hover:bg-blue-100 flex items-center justify-center p-2 rounded-md" />
          <Button type="text" onClick={() => handleOpenUpdate(record)} icon={<Edit size={16} className="text-blue-600" />} className="bg-blue-50 hover:bg-blue-100 flex items-center justify-center p-2 rounded-md" />
          <Button type="text" onClick={() => handleOpenDelete(record)} icon={<Trash2 size={16} className="text-red-500" />} className="bg-red-50 hover:bg-red-100 flex items-center justify-center p-2 rounded-md" />
          <Button type="text" onClick={() => handleOpenCauHinh(record)} icon={<Settings size={16} className="text-orange-500" />} className="bg-orange-50 hover:bg-orange-100 flex items-center justify-center p-2 rounded-md" />
        </Space>
      ),
    },
  ];
    ...
    <ConfigProvider theme={{ token: { colorPrimary: '#1d4ed8', borderRadius: 6 }, components: { Table: { headerBg: '#f8fafc', headerColor: '#334155', rowHoverBg: '#f1f5f9' } } }}>
      ...
      <div className="p-6 flex flex-col gap-8 bg-white min-h-[calc(100vh-200px)]">
            ...
            <div className="animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-2">
                <div className="flex flex-col gap-1.5">
                  <label className="text-gray-600 text-sm font-medium">Mã / Tên</label>
                  <Input placeholder="Nhập mã hoặc tên" className="h-10 w-full" value={searchKeyword} onChange={e => setSearchKeyword(e.target.value)} allowClear />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-gray-600 text-sm font-medium">Trạng thái</label>
                  <Select value={searchActive} onChange={setSearchActive} className="h-10 w-full" options={[{ value: 'all', label: 'Tất cả' }, { value: 'true', label: 'Hoạt động' }, { value: 'false', label: 'Không hoạt động' }]} />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-gray-600 text-sm font-medium">Ngày tạo</label>
                  <RangePicker className="h-10 w-full" placeholder={['Bắt đầu', 'Kết thúc']} format="DD/MM/YYYY" value={searchDates} onChange={setSearchDates} />
                </div>
              </div>
              <div className="flex justify-center mt-4">
                <Button type="primary" onClick={fetchData} className="bg-[#1d4ed8] hover:bg-[#1e40af] border-none px-8 h-10 font-medium">Làm mới dữ liệu</Button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex justify-between items-center mb-2">
            ...
            <Space>
              ...
              <Button danger className="border-red-500 text-red-500 h-10 font-medium px-4 hover:bg-red-50" disabled={selectedRowKeys.length === 0} onClick={handleOpenDeleteMultiple}>Xóa</Button>
            </Space>
          </div>
        ...
        <CreateSubjectCategoryModal
          ...
          onSave={async (values) => {
            try {
              await dmMonHocApi.create({ code: values.code!, name: values.name!, is_active: values.is_active ?? true, note: values.note ?? '' });
        ...
        <UpdateSubjectCategoryModal
          ...
          onSave={async (values) => {
            ...
            try {
              await dmMonHocApi.update(selectedRecord.id, { code: values.code, name: values.name, is_active: values.is_active, note: values.note });
        ...
        <DetailSubjectCategoryModal open={isDetailModalOpen} onClose={() => setIsDetailModalOpen(false)} record={selectedRecord} />
        ...
        <DeleteSubjectCategoryModal
          ...
          onConfirm={async () => {
            try {
              if (isDeleteMultiple) {
                await Promise.all(selectedRowKeys.map((k) => dmMonHocApi.delete(String(k))));
                messageApi.success(`Đã xóa ${selectedRowKeys.length} môn học!`);
                setSelectedRowKeys([]);
              } else if (selectedRecord) {
                await dmMonHocApi.delete(selectedRecord.id);
