...
import { dmKhoiLopApi, type DmKhoiLopAPI } from '../../../services/danhMucApi.ts';

const { RangePicker } = DatePicker;

/** Type khớp hoàn toàn với DB / API fields */
export interface DmKhoiLopType {
  id: string;
  code: string;
  name: string;
  is_active: boolean;
...
// ─── Inline modals ──────────────────────────────────────────────────
function CreateModal({ open, onClose, onSave }: { open: boolean; onClose: () => void; onSave: (v: Partial<DmKhoiLopType>) => Promise<boolean> }) {
  const [form, setForm] = React.useState<Partial<DmKhoiLopType>>({ is_active: true });
...
function UpdateModal({ open, onClose, record, onSave }: { open: boolean; onClose: () => void; record: DmKhoiLopType | null; onSave: (v: Partial<DmKhoiLopType>) => Promise<boolean> }) {
  const [form, setForm] = React.useState<Partial<DmKhoiLopType>>({});
...
function DetailModal({ open, onClose, record }: { open: boolean; onClose: () => void; record: DmKhoiLopType | null }) {
  return open && record ? (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-lg shadow-xl w-[560px] p-6">
        <div className="text-xl font-semibold text-slate-800 pb-3 border-b border-gray-200 mb-4">Chi tiết khối lớp</div>
        <div className="flex flex-col gap-3">
          {([['id','ID'],['code','Mã'],['name','Tên'],['note','Ghi chú'],['created_at','Ngày tạo'],['updated_at','Ngày cập nhật']] as [keyof DmKhoiLopType, string][]).map(([key, label]) => (
...
export default function DanhMucKhoiLop() {
  const [data, setData] = useState<DmKhoiLopType[]>([]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<DmKhoiLopType | null>(null);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try { const res = await dmKhoiLopApi.list(); setData(res.data as DmKhoiLopType[]); }
  ...
  const columns: ColumnsType<DmKhoiLopType> = [
