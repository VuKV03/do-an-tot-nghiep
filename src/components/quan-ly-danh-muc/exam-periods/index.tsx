...
import { dmDotThiApi } from '../../../services/danhMucApi.ts';
...
export default function DanhMucDotThi() {
  const [data, setData] = useState<DmDotThiType[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await dmDotThiApi.list();
        ...
        <UpdateDotThiModal
          ...
          onSave={async (values) => {
            try {
              ...
              await dmDotThiApi.update(selectedRecord!.Id, {
                code: values.Ma,
                name: values.Ten,
            ...
            try {
              if (isDeleteMultiple) {
                for (const key of selectedRowKeys) {
                  await dmDotThiApi.delete(key.toString());
                }
                message.success('Đã xóa các đợt thi được chọn!');
                setSelectedRowKeys([]);
              } else if (selectedRecord) {
                await dmDotThiApi.delete(selectedRecord.Id);
