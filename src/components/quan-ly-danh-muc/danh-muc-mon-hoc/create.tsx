...
export interface CreateSubjectCategoryModalProps {
  open: boolean;
  onClose: () => void;
  onSave?: (values: Partial<SubjectCategoryType>) => Promise<boolean> | boolean;
}

export default function CreateSubjectCategoryModal({
  open,
  onClose,
  onSave,
  ...
  return (
    <ConfigProvider theme={{ token: { colorPrimary: '#1d4ed8', borderRadius: 6 } }}>
      <Modal
        title={<div className="text-[20px] font-semibold text-slate-800 pb-3 border-b border-gray-200">Thêm mới môn học</div>}
        open={open}
