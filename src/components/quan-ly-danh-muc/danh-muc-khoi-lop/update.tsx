...
export interface UpdateKhoiLopModalProps {
  open: boolean;
  onClose: () => void;
...
export default function UpdateKhoiLopModal({ open, onClose, onSave, record }: UpdateKhoiLopModalProps) {
    ...
    >
      ...
      >
        ...
        >
          <Form.Item
            name="Ma"
            label={<span className="text-gray-700 font-medium text-[15px]">Mã khối lớp</span>}
            rules={[{ required: true, message: 'Vui lòng nhập mã khối lớp' }]}
          >
          ...
          <Form.Item
            name="Ten"
            label={<span className="text-gray-700 font-medium text-[15px]">Tên khối lớp</span>}
          ...
          >
            <TextArea 
              rows={4} 
              placeholder="Nhập ghi chú cho khối lớp." 
