...
import { Table, Input, Select, DatePicker, Button, Space, Typography, ConfigProvider } from 'antd';
import { ChevronDown, ChevronUp, Eye, Edit, Trash2 } from 'lucide-react';
import type { ColumnsType } from 'antd/es/table';
import CreateKhoiLopModal from './create';
import UpdateKhoiLopModal from './update';
import DetailKhoiLopModal from './detail';
import DeleteKhoiLopModal from './delete';

const { RangePicker } = DatePicker;

// Define type based on the provided DB schema
interface DanhMucKhoiLopType {
  Id: string;
  Ma: string;
  Ten: string;
  IsActive: boolean;
...
const mockData: DanhMucKhoiLopType[] = [
  { Id: '1', Ma: 'K12', Ten: 'Khối 12', IsActive: true, CreatedAt: '22-12-2024' },
  { Id: '2', Ma: 'K11', Ten: 'Khối 11', IsActive: false, CreatedAt: '22-12-2024' },
  { Id: '3', Ma: 'K10', Ten: 'Khối 10', IsActive: true, CreatedAt: '22-12-2024' },
  { Id: '4', Ma: '...', Ten: '...', IsActive: true, CreatedAt: '...' },
  { Id: '5', Ma: '...', Ten: '...', IsActive: false, CreatedAt: '...' },
];
...
export default function DanhMucKhoiLop() {
  ...
  const [selectedRecord, setSelectedRecord] = useState<DanhMucKhoiLopType | null>(null);

  const handleOpenDetail = (record: DanhMucKhoiLopType) => {
    setSelectedRecord(record);
    setIsDetailModalOpen(true);
  };

  const handleOpenUpdate = (record: DanhMucKhoiLopType) => {
    setSelectedRecord(record);
    setIsUpdateModalOpen(true);
  };

  const handleOpenDelete = (record: DanhMucKhoiLopType) => {
    setSelectedRecord(record);
  ...
  const columns: ColumnsType<DanhMucKhoiLopType> = [
    {
      title: 'STT',
      dataIndex: 'stt',
      key: 'stt',
  ...
  return (
    <ConfigProvider
      theme={{
        token: {
          colorPrimary: '#1d4ed8', // Tailwind blue-700
          borderRadius: 6,
        },
        components: {
          Table: {
            ...
            <div className="animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-2">
                <div className="flex flex-col gap-1.5">
                  <label className="text-gray-600 text-sm font-medium">Mã/ tên khối lớp</label>
