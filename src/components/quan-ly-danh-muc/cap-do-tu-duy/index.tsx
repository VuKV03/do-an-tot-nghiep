...
function CreateModal({ open, onClose, onSave }: { open: boolean; onClose: () => void; onSave: (v: Partial<CognitiveLevelType>) => Promise<boolean> }) {
  ...
  const handleSave = async () => {
    const nextErrors = validateCodeName(form, 'cấp độ tư duy');
  ...
  return open && record ? (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-lg shadow-xl w-[560px] p-6">
        ...
        <div className="flex flex-col gap-4">
          {([['code','Mã cấp độ'],['name','Tên cấp độ'],['note','Ghi chú'],['created_at','Ngày tạo'],['updated_at','Ngày cập nhật']] as [keyof CognitiveLevelType, string][]).map(([key, label]) => {
...
export default function DanhMucCapDoTuDuy() {
  ...
  const { colGroup: capDoColGroup, startResize: startCapDoColResize, totalWidth: capDoTableTotalWidth } = useResizableColumns(
  ...
  const excelColumns: ExcelColumn<CognitiveLevelType>[] = [
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(filteredData, excelColumns, fileName, 'Cấp độ tư duy');
  ...
  return (
    <ConfigProvider theme={{ token: { colorPrimary: '#1d4ed8', borderRadius: 6 }, components: { Table: { headerBg: '#f8fafc', headerColor: '#334155', rowHoverBg: '#f1f5f9' } } }}>
        ...
        <div className="flex flex-col gap-4">
          ...
          <Spin spinning={loading}>
            <div className="overflow-x-auto border-t border-gray-200">
              <table style={{ minWidth: capDoTableTotalWidth }} className={`w-full text-sm text-slate-700 border-collapse table-fixed ${RESIZABLE_TABLE_CLASS}`}>
                {capDoColGroup}
                <thead>
                  <tr className="bg-[#f8fafc] border-b border-gray-200 text-[#334155] font-semibold">
                    <th className="relative py-3 px-3 text-center">
                      ...
                      <ColResizeHandle onMouseDown={startCapDoColResize(0)} />
                    </th>
                    <th className="relative py-3 px-3 text-center">STT<ColResizeHandle onMouseDown={startCapDoColResize(1)} /></th>
                    <th className="relative py-3 px-3 text-left">Mã<ColResizeHandle onMouseDown={startCapDoColResize(2)} /></th>
                    <th className="relative py-3 px-3 text-left">Tên<ColResizeHandle onMouseDown={startCapDoColResize(3)} /></th>
                    <th className="relative py-3 px-3 text-left">Ghi chú<ColResizeHandle onMouseDown={startCapDoColResize(4)} /></th>
                    <th className="relative py-3 px-3 text-left">Ngày tạo<ColResizeHandle onMouseDown={startCapDoColResize(5)} /></th>
                    <th className="relative py-3 px-3 text-center">Thao tác<ColResizeHandle onMouseDown={startCapDoColResize(6)} /></th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {paginatedData.length === 0 ? (
                    <tr><td colSpan={7} className="py-12 text-center"><Empty description="Không có dữ liệu cấp độ tư duy" /></td></tr>
      ...
      <CreateModal open={isCreateOpen} onClose={() => setIsCreateOpen(false)}
        onSave={async (v) => {
          try { await cognitiveLevelApi.create({ code: v.code!, name: v.name!, note: v.note ?? '' }); toast.success('Thêm thành công!'); fetchData(); return true; }
          catch (error: any) { toast.error(error.message || 'Lỗi khi thêm!'); return false; }
        }}
      />
      <UpdateModal open={isUpdateOpen} onClose={() => setIsUpdateOpen(false)} record={selectedRecord}
        onSave={async (v) => {
          if (!selectedRecord) return false;
          try { await cognitiveLevelApi.update(selectedRecord.id, { code: v.code, name: v.name, note: v.note }); toast.success('Cập nhật thành công!'); fetchData(); return true; }
          catch (error: any) { toast.error(error.message || 'Lỗi khi cập nhật!'); return false; }
        }}
      />
      ...
      <DeleteModal open={isDeleteOpen} onClose={() => setIsDeleteOpen(false)} itemName={selectedRecord?.name} isMultiple={isDeleteMultiple} multipleCount={selectedRowKeys.length}
        onConfirm={async () => {
          if (isDeleteMultiple) {
            ...
            if (failed.length === 0) {
              toast.success(`Đã xóa ${succeeded.length} cấp độ!`);
