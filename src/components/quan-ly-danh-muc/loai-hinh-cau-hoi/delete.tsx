...
export interface DeleteLoaiHinhCauHoiModalProps {
  open: boolean;
  onClose: () => void;
  onConfirm: () => void;
  itemName?: string;
...
export default function DeleteLoaiHinhCauHoiModal({ open, onClose, onConfirm, itemName, isMultiple, multipleCount }: DeleteLoaiHinhCauHoiModalProps) {
  return (
    <Modal
      title={
      ...
      <div className="flex items-start gap-4 mb-8 px-2">
        ...
        <div className="text-[17px] text-slate-800 pt-1">
          {isMultiple
            ...
            : `Bạn có chắc chắn xóa bản ghi có tên loại hình câu hỏi "${itemName || ''}"?`
          }
        </div>
      </div>
